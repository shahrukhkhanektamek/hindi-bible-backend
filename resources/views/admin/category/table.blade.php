                    ...
                    <td>
                        <div class="d-flex gap-2 align-items-center">
                            ...
                            <div class="flex-grow-1">
                                ...
                                @if(!empty($check))
                                    <a href="{{route('sub-category.list')}}?id={{Crypt::encryptString($value->id)}}">{{$value->name}}</a>
                                @else
                                    <a href="{{route('post.add')}}?category_id={{Crypt::encryptString($value->id)}}">{{$value->name}}</a>
                                @endif
                            </div>
                        </div>
                    </td>
