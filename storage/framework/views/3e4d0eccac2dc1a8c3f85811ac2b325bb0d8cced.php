...
<?php ($sub_sub_category = DB::table("sub_sub_category")->where("id", Crypt::decryptString(request()->id))->first()); ?>
<?php endif; ?>

<?php if(!empty(request()->id)): ?>
<?php ($sub_category = DB::table("sub_category")->where("id", @$sub_sub_category->sub_category_id)->first()); ?>
<?php endif; ?>



<?php if(!empty(request()->id)): ?>
<?php ($category = DB::table("category")->where("id", @$sub_category->category_id)->first()); ?>
<?php endif; ?>
                                        ...
                                        <div class="col-md-2">
                                         ...
                                         <div class="col-md-2">
                                            <select class="form-control" data-href="" data-target="#sub-sub-sub-category" id="sub-sub-category" required>
                                                ...
                                                <?php if(!empty($sub_sub_category)): ?>
                                                    <option value="<?php echo e(@$sub_sub_category->id); ?>"  selected ><?php echo e($sub_sub_category->name); ?></option>
                                                <?php endif; ?>
                                            </select>
                                         </div>
...
<script>
   ...
   {
        var sub_sub_category = $("#sub-category").val();
       var status = $("#statuschange").val();
       var order_by = $("#order_by").val();
       var limit = $("#limit").val();
       var filter_search_value = $(".search-input").val();
       data = `status=${status}&order_by=${order_by}&limit=${limit}&filter_search_value=${filter_search_value}&sub_sub_category=${sub_sub_category}`;
   }
    get_url_data();
   url = main_url+'?'+data;
   load_table();
   $(document).on("change", "#statuschange, .order_by, .limit, #sub-sub-category, #sub-category, #category, #sub-sub-sub-category",(function(e) {
      get_url_data();
      url =main_url+"?"+data;
      load_table();
   }));
...
</html><?php /**PATH D:\xamp\htdocs\projects\codediffusion\hindibible\resources\views\admin\sub-sub-sub-category\index.blade.php ENDPATH**/ ?>